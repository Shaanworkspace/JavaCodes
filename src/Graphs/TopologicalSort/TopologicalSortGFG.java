...
public class TopologicalSortGFG {
	...
	public ArrayList<Integer> topoSortMethod1(int V, int[][] edges) {
		...
		for (int i = 0; i < V; i++) {
			if (!vis[i]) topology(i, res, adjM, vis);
		}
		Collections.reverse(res);
		return res;
	}

	private void topology(int node, ArrayList<Integer> res, List<List<Integer>> adjM, boolean[] vis) {
		vis[node] = true;
		for (int i : adjM.get(node)) {
			if (!vis[i]) {
				topology(i, res, adjM, vis);
			}
		}
		res.add(node);
	}
	...
	public ArrayList<Integer> topoSortMethod2(int V, int[][] edges) {
		...
		for (int a : indeg) System.out.println(a);

		boolean[] vis = new boolean[V];

		ArrayList<Integer> res = new ArrayList<>();
		for (int i = 0; i < V; i++) {
			if (!vis[i]) bfs(i, indeg, res, adjM, vis);
		}
		return res;
	}

	private void bfs(int node, int[] indeg, ArrayList<Integer> res, List<List<Integer>> adjM, boolean[] vis) {
